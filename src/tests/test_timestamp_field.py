...
class TestTimestampField(django.test.TransactionTestCase):
    """
    Since Django test cases automatically apply the INSTALLED_APPS models
    and migrations, any syntax violations in the SQL generated by the models or
    their fields will trigger database-level errors before the test cases are
    ...
    This class inherits from TransactionTestCase so that I can test database-
    level exceptions without causing the "You can't execute queries until the
    end of the 'atomic' block." Django exception message. This error was still
    thrown even when calling self.assertRaises and wrapping that in its own,
    explicit transaction.atomic() block. One more ORM complication.
    ...
    def _test_insert_values(self, test_model_class, values_dict):
        ...
        This is part of a test of the INSERT operations with the timestamp
        field. This method is defined separately to break up deep nesting of
        blocks.

        Args:
            test_model_class (tests.models.*) Test Django model class.
            values_dict (dict): The timestamp field test config attribute
                containing model field attribute values and expected database
                values fetched after insert.
        ...
        for alias in test_utils.get_db_aliases():
            ...
            with self.subTest(backend=engine):
                ...
                if class_expected and issubclass(expected_value, Exception):
                    ...
                    retrieved_record_model = test_model.__class__.objects \
                        .using(alias).get(id=test_model.id)
    ...
    def _test_update_insert_only(self, alias):
        ...
        This is part of a test of the UPDATE operations with the timestamp
        field.
    ...
    def test_sqlite3_table_structure(self):
        ...
        A sqlite syntax exception is raised if one attempts to pass parameters
        to cursor.execute().
